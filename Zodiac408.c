...
void swapColumns(int data[], int rows, int rowLen, int i, int j);
void printColumns(int data[], int rowLen, int dataLen);
...
int main(int argc, char *argv[]) {
    ...
    {
        /* Create plain model */
        sg_hmm_t *mod2 = sg_hmm_Create();
        mod2->N = 26;
        mod2->M = 54;
        
        sg_hmm_InitRandomWithoutA(mod2);
        mod2->A = A;
        sg_trainWithoutA(mod2, obs2, 200);   
        double score =  _computeLogScore(obs2);
        if(best_model_score < score) {
            best_model_score = score;
        }
        printf(".");
...
//     for (int i = 0; i < 408; ++i)
//     {
//         if(key[cipher_text[i]] == plain_text[i]) {
//             correct++;
//         }
//     }
// 	return correct / 408;
